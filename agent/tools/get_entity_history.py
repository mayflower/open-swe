...
async def aget_entity_history(
    entity_id: str,
    include_deep_history: bool = False,
) -> dict[str, Any]:
    """Async sibling — awaits the store reads so the agent loop never blocks
    on entity / event fetches.
    """
    runtime = resolve_runtime_from_context()
    repo = runtime_attr(runtime, "repo", "unknown")
    store = runtime_attr(runtime, "store")
    if store is None:
        return {"status": "unavailable", "entity_id": entity_id}
    if hasattr(store, "aget_entity"):
        entity = await store.aget_entity(entity_id)
    else:
        entity = store.get_entity(entity_id)
    if entity is None:
        return {"status": "not_found", "entity_id": entity_id}
    if hasattr(store, "alist_repo_events"):
        ...
        repo_events = store.list_repo_events(repo)
    events = search_repo_events(repo_events, entity.current_revision.qualified_name)
    deep_history = maybe_load_deep_history(
        include_deep_history,
        lambda: load_entity_git_history(runtime, entity.current_revision.path),
    )
