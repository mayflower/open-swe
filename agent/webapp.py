...
def build_jira_prompt_text(
    ...
    *,
    ...
    triggered_by_line = (
        f"## Triggered by: {triggering_user_name}\n\n" if triggering_user_name else ""
    )
    comments_text = ""
    if comments:
        comments_text = "\n\n## Comments:\n"
...
async def process_jira_issue(issue_data: dict[str, Any], repo_config: dict[str, str]) -> None:
        ...
        relevant_comments.append(
            {
                ...
                "user": {
                    "name": triggering_comment.get("author", {}).get(
                        "displayName", user_name or "User"
                    )
                },
            }
        )
    ...
    project_key = fields.get("project", {}).get(
        "key", issue_key.split("-", 1)[0] if "-" in issue_key else ""
    )
...
def verify_jira_signature(body: bytes, signature: str, secret: str) -> bool:
    ...
    expected = hmac.new(secret.encode("utf-8"), body, hashlib.sha256).hexdigest()
    normalized_signature = signature.removeprefix("sha256=") if isinstance(signature, str) else ""
