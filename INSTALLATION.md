...
This guide walks you through setting up Open SWE end-to-end: local development, GitHub App creation, LangSmith configuration, sandbox provider selection, webhooks, the web dashboard, and production deployment.
...
4. Update your webhook URLs (Linear, Jira, Slack, GitHub App) and the GitHub App / Slack OAuth callback URLs to your production URLs (replace the ngrok / localhost values). The dashboard GitHub App callback must be `<DASHBOARD_API_BASE_URL>/dashboard/api/auth/callback`.
