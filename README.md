...
- **`check_message_queue_before_model`** — Injects follow-up messages (Linear/Jira comments or Slack messages that arrive mid-run) before the next model call. You can message the agent while it's working and it'll pick up your input at its next step.
