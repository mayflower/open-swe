...
class _FakeBackend:
    def __init__(
        self, sandbox_id: str, health_result: _ExecuteResult | Exception | None = None
    ) -> None:
        self.id = sandbox_id
...
async def test_check_or_recreate_sandbox_recreates_on_any_health_exception(provider: str) -> None:
    ...
    mock_recreate.assert_called_once_with("thread-123", github_proxy_token=None)
...
async def test_check_or_recreate_sandbox_recreates_on_nonzero_health_exit(provider: str) -> None:
    stale_backend = _FakeBackend(
        "stale", health_result=_ExecuteResult(exit_code=1, output="not ok")
    )
    replacement_backend = _FakeBackend("replacement")
...
@pytest.mark.asyncio
async def test_get_agent_recreates_unhealthy_cached_sandbox_before_refreshing_github_access() -> (
    None
):
