...
class _FakeAgentSandboxBackend:
    ...
    @classmethod
    def from_existing(
        cls, sandbox: object, *, root_dir: str | None = None
    ) -> _FakeAgentSandboxBackend:
...
def _install_fake_agent_sandbox_sdks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    k8s_module.GatewayConnectionConfig = lambda *, gateway_name, gateway_namespace: (
        _FakeConnectionConfig(
            "gateway",
            {
...
def test_provider_registration_accepts_agent_sandbox(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda sandbox_type: lambda sandbox_id=None: sentinel,
    )
